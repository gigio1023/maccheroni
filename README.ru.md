...
Корейский и итальянский — первые два языковых профиля. В качестве следующего набора фикстур подготовлен закреплённый корейско-английский приёмочный пакет — фрагменты HiKE с переключением кодов и встреча AMI с четырьмя говорящими — и новые измерения добавляются в эту таблицу по мере появления.
...
Выигрыш от коррекции измеряется, а не предполагается. Четырёхрежимный стенд сравнения оценивает сырой и исправленный вывод завершённого запуска по одному эталону, с инъекцией глоссария при декодировании и без неё, и считает каждое применённое исправление, отдалившее сегмент от эталона. Уверенно неверное исправление не может спрятаться в среднем значении.

Завершённый запуск не заморожен. `maccheroni postprocess` и приложение выводят новые наборы исправлений или перевода из запечатанных сегментов без повторного запуска ASR, а каждое сохранение глоссария хранится как контентно-адресуемая ревизия, поэтому последующий вывод может использовать в точности те байты глоссария, которые записал исходный запуск.
...
После успешной сборки, инвентаризации разрешённых ресурсов и строгих проверок подписи кода приложение выводит путь к своему bundle. Веса моделей загружаются при первом использовании. Исполняемый файл не содержит веса моделей или среды Python; `maccheroni doctor` проверяет среды выполнения, закреплённые снимки и готовность хранилища для каждого настроенного тома.
...
| `Tests/` | 315 тестов на основе fixtures в 24 наборах |
