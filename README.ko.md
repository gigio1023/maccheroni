...
Maccheroni는 개인용 도구입니다. 한 대의 Mac에서 혼용 언어 대화를 화자가 표시된 기록으로 바꾸는 구성 가능한 작업대입니다. 시장을 겨냥하지 않으며 사용자가 자신의 용도에 맞춰 조정합니다.

- **프로필은 대화마다 설정을 구성합니다.** 각 프로필은 대화의 실제 소리에 맞춰 고정된 기기 내 ASR 모델, 용어집 context, 전체 파일 화자 처리를 조합합니다. 영어 제품명이 많이 나오는 한국어 회의와 이탈리아어 2인 대화에는 서로 다른 구성이 필요합니다.
- **조합은 측정해서 정합니다.** 공개 fixture와 합성 fixture, 용어 재현율, 오류율을 이 저장소에 기록합니다. 모델이나 용어집을 바꿀 때 같은 비교를 다시 실행할 수 있으므로 감으로 판단하지 않습니다.
- **모든 run은 증거를 남깁니다.** 고정된 모델 revision, 용어집 전달 기록, raw 전사, 화자 timeline, typed failure를 run에 봉인합니다. 나중에 결과를 검토하고 재현할 수 있습니다.

이 선택의 바탕이 된 소스 수준 기록은 [docs/reference-project-source-audit.md](docs/reference-project-source-audit.md)에 있습니다.
